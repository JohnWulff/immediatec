...
#
#  Modification John Wulff 2023.08.25 - add code to connect with an MQTT broker
#	https://metacpan.org/pod/Net::MQTT::Simple
#	copy iCmsg.pm to iCmsgM.pm for use in iCmqtt for communication with MQTT
#	rename package to iCmsgM in file iCmsgM.pm because of the many changes
#
########################################################################
...
sub connect {
    ...
    print STDERR "iCmsgM.pm:connect\n" if $trace_rcv or $trace_send;
    return $conn;
}
...
sub disconnect {
    ...
    print STDERR "iCmsgM.pm:disconnect\n" if $trace_rcv or $trace_send;
...
sub send_now {
    my ($conn, $msg) = @_;
    print STDERR "iCmsgM.pm:send_now    $msg\n" if $trace_send;
    _enqueue ($conn, $msg);
...
sub send_later {
    my ($conn, $msg) = @_;
    print STDERR "iCmsgM.pm:send_later  $msg\n" if $trace_send;
    _enqueue($conn, $msg);
    my $sock = $conn->{sock};
    return unless defined($sock);
...
sub _enqueue {
    ...
    my $len = length($msg);
    print STDERR "iCmsgM.pm:_enqueue [$len]$msg\n" if $trace_send;
    $msg = pack ('N', $len) . $msg;
...
sub _send {
	...
	print STDERR "iCmsgM.pm:_send*0  [$l]$m sendBusy $conn->{sendBusy}\n" if $trace_send;
    ...
    while (@$sq) {
	    ...
	    my ($l, $m) = unpack "N A*", $msg;
	    print STDERR "iCmsgM.pm:_send*1  [$l]$m\n" if $trace_send;
            $bytes_written = syswrite ($sock, $msg,
                                       $bytes_to_write, $offset);
	    print STDERR "iCmsgM.pm:_send*2  [$l]$m\n" if $trace_send;
    ...
    print STDERR "iCmsgM.pm:_send*end	sendBusy $conn->{sendBusy}\n" if $trace_send and $conn->{sendBusy} > 1;
...
sub handle_send_err {
   # For more meaningful handling of send errors, subclass iCmsgM and
   # rebless $conn.
   my ($conn, $err_msg) = @_;
...
my $main_socket = 0;
sub new_server {
    @_ == 4 || die "iCmsgM->new_server (myhost, myport, login_proc)\n";
    ...
    print STDERR "iCmsgM.pm:new_server\n" if $trace_rcv or $trace_send;
...
sub rcv_now {
    ...
    print STDERR "iCmsgM.pm:rcv_now     $msg\n" if $trace_rcv;
...
sub _rcv {                     # Complement to _send
	...
	print STDERR "iCmsgM.pm:_rcv*0   [$offset]$msg\n" if $trace_rcv;
    } else {
        # The typical case ...
    ...
    while ($bytes_to_read) {
	print STDERR "iCmsgM.pm:_rcv*1   [$bytes_to_read]$msg\n" if $trace_rcv;
		...
		print STDERR "iCmsgM.pm:_rcv*x   [$bytes_to_read]$msg\n" if $trace_rcv;
    ...
    print STDERR "iCmsgM.pm:_rcv*2   [$offset]$msg\n" if $trace_rcv;
  ...
  FINISH:
    ...
    if ($conn->{rcvBusy}++) {
	my $m = $rq->[0];
	print STDERR "iCmsgM.pm:_rcv*0       $m	rcvBusy $conn->{rcvBusy}\n" if $trace_rcv;
	...
	my $msg = shift @$rq;
	print STDERR "iCmsgM.pm:_rcv*3       $msg\n" if $trace_rcv;
	&{$conn->{rcvd_notification_proc}}($conn, $msg, $!);
    }
    print STDERR "iCmsgM.pm:_rcv*end		rcvBusy $conn->{rcvBusy}\n" if $trace_rcv and $conn->{rcvBusy} > 1;
...
sub _new_client {
    ...
    print STDERR "iCmsgM.pm:_new_client\n" if $trace_rcv or $trace_send;
}
...
sub event_loop {
    my ($pkg, $mqtt) = @_; # event_loop($mqtt) enables tick()
    my ($r, $w, $rset, $wset);
    my $time_out = 0.1;
    while (1) {
	$mqtt->tick();
##	print "event_loop:\n";
	...
	if ($rd_handles->count() || $wt_handles->count()) {
	    ($rset, $wset) =
